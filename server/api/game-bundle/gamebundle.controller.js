...
exports.claim = function(req, res) {
	...
	gamebundle.findOne({
		...
		if (!found) return handleError(res, {
			error: ' redemption code is not avaliable or has been claimed'
		});
...
function process_threshold(bundlename) {
	...
	gamebundle.findOne({
		...
		gamebundle.aggregate({
			...
			var unclaimed = bundle[0].hasOwnProperty('unclaimed') ? bundle[0].unclaimed : 0;

			if (unclaimed / total * 100 <= threshold)
				email.send({
					text: bundlename + ' is below threshold',
