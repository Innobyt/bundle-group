...
exports.submit = function(req, res) {
	...
	gameredemption.findOne({
		...
		gamebundle.post.claim(req.body, function(err, gamebundle_result) {
			...
			// handle not found
			if (!gamebundle_result) return handleError(res, {
				message: ' could not redeem key'
			});
			...
			async.map(gamerepo_request, AsyncLibrary.each.bind(AsyncLibrary), function(err, result) {
				...
				// handle not found
				if (!result) return handleError(res, {
					message: ' could not find gametitles'
				});
